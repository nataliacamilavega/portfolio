...
const ProjectItem = ({
  ...
  const renderSkillTags = skillsTag.map((e, index) => (
    <SkillTag key={e + index}>{e}</SkillTag>
  ));

  const renderText = text.map((e, index) => (
    <PList key={e + index}>
      <Sparkle aria-hidden="true" style={{ color: `#ffc107` }} />
      {e}.
    </PList>
  ));
    ...
    <Container>
      ...
      <CardContainer>
        <img
          ...
          src={`/assets/project-cover/` + image}
          width="800"
        />
        <ColumnContainer>
          ...
          {comment && <CommentP>{comment.trim()}</CommentP>}
        </ColumnContainer>
      </CardContainer>
    </Container>
  );
